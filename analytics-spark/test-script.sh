...
docker exec -t $SPARK_ID /usr/local/spark/bin/spark-submit \
  --packages org.infinispan:infinispan-spark_2.11:$CONNECTOR_VERSION \
  --conf spark.io.compression.codec=lz4 \
  --class test.SimpleSparkJob /usr/local/code/target/analytics-spark-1.0-SNAPSHOT.jar $INFINISPAN_ADDRESS
